                    ...
                    if (diffs.some(diff => diff > 0)){
                       q_level.forEach(q =>{
                           ...
                           if(BM_prices[q] && CA_prices[q] && diffs[q] && diffs[q] > 0) {
                               worthyOrder.push(itemname,BM_prices[q]);
                            } else {
...
function getRecipe(itemname){
    var test_item="T2_BAG";
    let url = `https://gameinfo.albiononline.com/api/gameinfo/items/${test_item}/data`;
    ...
    //        console.log('body : '+body);
            item_compo.push(JSON.parse(body));
            return item_compo;
        }else{
            //console.log(response.statusCode);
        }
